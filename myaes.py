...
class MYAES:
    ...
    def myaes_encrypt(self, plaintext, sym_key):
        plaintext_bytes = self.pad(plaintext).encode()
        #print(b'plaintext_bytes = ' + plaintext_bytes)
        n = len(plaintext_bytes) / 16
        full_text = bytes()

        #生成轮密钥
        sym_key = int.from_bytes(sym_key, byteorder='big')
        RoundKeys = self.round_key_generator(sym_key)
        for i in range(int(n)):
            # 将明文拆分成块
            gadget = plaintext_bytes[16*i:16*(i+1)]
            #print(gadget)
            #对每一个明文块进行加密
            en_gadget = self.aes_encrypt(gadget, RoundKeys)
            #拼接密文块
            full_text += bytes(en_gadget)
        #print(b'full_text = ' + full_text)
        return full_text

    def myaes_decrypt(self, ciphertext, sym_key):
        n = len(ciphertext) / 16
        full_text = bytes()
        ...
        RoundKeys = self.round_key_generator(sym_key)
        for i in range(int(n)):
            gadget = ciphertext[16*i:16*(i+1)]
            #print(gadget)
            #对每一个密文块进行解密
            de_gadget = self.aes_decrypt(gadget, RoundKeys)
            #拼接解密出来的明文块
            full_text += bytes(de_gadget)
        return self.unpad(full_text.decode())
